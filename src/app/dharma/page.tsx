      ...
      <div className="relative z-10 gradient-bg min-h-screen">
        ...
        <nav className="flex items-center justify-between px-6 py-5 max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4 text-sm text-[var(--text-secondary)]">
            ...
            <Link href="/dharma" className="text-[var(--text-accent)] transition-colors hidden sm:inline">
              ☸ 法藏
            </Link>
            <span className="opacity-50 hidden md:inline">·</span>
            <span className="opacity-50 hidden md:inline">寂如师父 · 佛法修学</span>
          </div>
        </nav>
        ...
        <section className="px-6 pt-16 pb-8 text-center max-w-4xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight leading-tight mb-6">
            ☸ 法藏
            <br />
            <span className="text-[var(--text-accent)]">Dhārma Treasury</span>
          </h1>
        ...
        <section className="px-6 pb-12 max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {entryCards.map((card, i) => {
              ...
              const CardContent = (
                <div
                  className={`group p-6 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] transition-all duration-300 ${isDisabled ? 'opacity-60 cursor-not-allowed' : 'hover:border-[var(--text-accent)]/40 cursor-pointer'}`}
                  style={!isDisabled ? { background: card.color, borderColor: card.borderColor } : {}}
                >
                  <div
                  ...
                  <h3 className="text-lg font-bold mb-1 text-[var(--text-primary)]">
                    {card.title}
                    {isDisabled && <span className="ml-2 text-xs text-[var(--text-secondary)] opacity-60">(Phase 2)</span>}
                  </h3>
        ...
        <section className="px-6 pb-12 max-w-3xl mx-auto text-center">
          <h2 className="text-lg font-bold mb-6 text-[var(--text-accent)]">✦ 随机法语</h2>
          <div
          ...
          >
            ...
            <p className="text-xs text-[var(--text-secondary)] mt-4 opacity-50">点击刷新法语</p>
          </div>
        </section>
        ...
        <section className="px-6 pb-12 max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {Object.entries(categoryInfo).map(([key, info]) => (
              <div
                key={key}
                className="p-5 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] text-center"
              >
        ...
        <footer className="border-t border-[var(--border-color)] py-8 px-6">
          <div className="max-w-5xl mx-auto text-center text-sm text-[var(--text-secondary)] opacity-40">
            ...
            <p className="mt-2">
              ...
              <Link href="/dharma/treasury" className="hover:text-[var(--text-accent)] transition-colors opacity-30 hover:opacity-60">
                浏览全部主题
              </Link>
            </p>
          </div>
